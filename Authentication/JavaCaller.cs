...
namespace MCMicroLauncher.Authentication
{
    internal class JavaCaller
    {
        ...
        internal JavaCaller(StateMachine<State, Trigger> stateMachine)
        {
            this.StateMachine = stateMachine;
        }

        internal void LaunchMinecraft(
            string username,
            string uuid,
            string accessToken,
            bool borderlessFullscreen)
        {
            var arguments = @"""-Dos.name=Windows 10"" -Dos.version=10.0 -XX:HeapDumpPath=MojangTricksIntelDriversForPerformance_javaw.exe_minecraft.exe.heapdump "
                   ...
                   + $@"-Xms3G -Xmx3G -d64 -XX:+DisableExplicitGC {(borderlessFullscreen ? "-Dorg.lwjgl.opengl.Window.undecorated=true" : "")} "
                   ...
                   + $@"--uuid {uuid} --accessToken {accessToken} --userType mojang --tweakClass net.minecraftforge.fml.common.launcher.FMLTweaker --versionType Forge ";
